...
not_prog_mfcc_dir = os.path.join(currdir, "Test_MFCC\\Not_Progressive_Rock")
other_mfcc_dir = os.path.join(currdir, "Test_MFCC\\Other")
prog_mfcc_dir = os.path.join(currdir, "Test_MFCC\\Progressive_Rock")
...
for filename in os.listdir(not_prog_mfcc_dir): 
    ...
    for n in range(int(curr_data.shape[1]/20)):
        X.append(curr_data[:,n*20:(n+1)*20])
        Y.append([0,1])
...
for filename in os.listdir(other_mfcc_dir): 

    X = []
    Y = []

    print("Reading song: " + filename)
    csv_file_path = os.path.join(other_mfcc_dir, filename)
